...
趣灵的主线已经收束为 AI 知识轻消费产品。当前源码进入公开测试基础版：默认开放五个精选静态 Flow，动态生成通过邀请/开放模式受控放量；V7 仍只作为后续质量护栏。

## 必读文档

| 文档 | 用途 |
|---|---|
| `PUBLIC_BETA.md` | 公开测试模式、持久化、配额、隐私、指标、上线与回滚 |
...
- 默认公开体验：Explore 精选主题 -> Flow 闯关 -> 理解反馈/后续分支 -> Hub 回顾。
- 受控动态体验：邀请或开放模式下，自由输入 -> 生成阶段 -> 四步路径预览 -> Flow。
...
| RELEASE_AUDIT_2026-07-17.md | 线上完整闭环、发布问题分级、修复状态和 5 人无讲解试玩记录表。 |

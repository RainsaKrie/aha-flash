...
> 公开测试口径（2026-07-17）：默认只开放五个精选静态 Flow；动态生成通过 `static / invite / open` 三档模式逐步放量。公开测试层只控制访问、成本、缓存、匿名指标和反馈，不扩展内容引擎。
## 1. 产品定位
...
**公开入口包含**：
- 五个精选 topic 卡片：默认公开、直接进入 Flow、零模型调用。
- 自由输入框：静态模式禁用并标明“小范围内测”；邀请模式需要有效邀请码；开放模式才允许直接动态生成。
...
**冷启动策略**：默认先让用户完成五条稳定垂直切片；动态生成从邀请码小流量开始，确认成本、成功率、完成率和反馈后再决定是否开放。
...
**内容来源**：
- 公开默认：五条已验证的静态 Flow，不消耗模型预算。
- 小范围动态：LLM 实时生成 + Eval/QualityGate + 持久化配额、token 预算与成功缓存。
...
- [x] 线上发布审计：真实动态生成可完成并写入 Hub；完成页补 Hub 入口；动态时长与歧义单选进入 QualityGate
- [ ] 5 位外部用户无讲解试玩：只修重复出现或阻断完成的问题
...
第一轮只用 20-30 次有效会话建立趣灵自己的基线，不预设行业标准，也暂不把次日留存当作本轮决策条件。

| 核心指标 | 定义 | 回答的问题 |
|---|---|---|
| 开始率 | flow_started 独立用户 / page_view 独立用户 | 陌生人是否愿意开始 |
| 完成率 | flow_completed 独立用户 / flow_started 独立用户 | Flow 是否让人愿意走完 |
| 连续探索率 | second_flow_started 独立用户 / 首条 flow_completed 独立用户 | 产品是否不只像一次性 Demo |

辅助观察包括每关退出率、下一话题点击率、Hub 打开率和三档理解反馈。动态模式另外观察生成成功、token、估算成本、repair/retry/fallback、缓存命中、平均与 P95 延迟、限流和预算耗尽。

漏斗解释：

- 开始率低：先检查入口、话题钩子和定位。
- 开始率高但完成率低：检查 Flow 内容、文案或交互断点。
- 完成率高但连续探索率低：检查完成后的分支是否让人想继续。
- 动态成功但成本高：优先增加缓存命中、静态精选和更严格配额，不扩展引擎。
...
### V6.1 通用Skill与事实边界（2026-06-24）

- 当前的8个Structure Skill只负责教学结构和交互能力边界，不存放某个具体概念的预制课程，也不会把内部步骤词直接展示给用户。
- 用户输入的主题术语由ConceptPlan提取，四步教学序列由KnowledgeBlueprint组织，具体交互由Pattern组件实现；这三层不能互相越权。
- 数值模拟仅在存在可验证公式时使用；没有公式的主题应改用比较、选择、连线、排序或回忆等交互，不能生成看似精确的趋势数字。
- 事实准确性不是V6通过增加更多专用Skill来解决的。V7再引入按需Evidence Pack：仅对需要事实、日期、公式或时效信息的请求检索权威来源，并将来源和可用声明传给生成链路。
<!-- CURRENT_STATUS_END -->
